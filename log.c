...
#include <unistd.h>                                                 // Обеспечивание доступа к API операционной системы POSIX                                                                    
//
...
int barrier;                                                        // Барьер для архивации
//
// ДЕКЛАРАЦИЯ ПРОЦЕДУР
//
...
int file_exists()                                                   // Проверка существования файла
{
    ...
    memset(&info, 0, sizeof(info));                                 // Инициализация структуры нулевыми значениями
...
{
    // Декларация переменных
    struct statvfs statv;                                           // Структура для хранения информации о файловой системе
    ...
    unsigned long long size;                                        // Хранение размера файла
    struct stat st;                                                 // Хранит информацио о файле

    // Инициализация переменных
    memset(&statv, 0, sizeof(statv));                               // Инициализация структуры для хранения информации о файловой системе
    memset(&archive, 0, sizeof(archive));                           // Инициализация массива символов для хранения команды архивации файла
    memset(&st, 0, sizeof(st));                                     // Инициализация информации о файле
    free_space = 0;                                                 // Инициализация переменной для хранения размера свободного пространства на диске
    size = 0;                                                       // Инициализация переменной размера
    ...
    {                                                                               
        ...
        } else if((free_space < 1024*1024) & (barrier == 1))
        {
            ...
            // printf("Выход из disk_space_check, 1\n");            // DEBUG
            return 2;                                               // Возвращаем код ошибки
...
int diag_log()                                                      // Вывод диагностического сообщения
{
    ...
    switch (error_code_log) 
    {
        ...
        case 4:
            ...
            break;
        case 5:
            printf("Не удалось получить доступ к файлу\n");
            break;
    }
